...
class AdminService {
  ...
  async getAdminStats(): Promise<AdminStats> {
    const response = await httpClient.get(`${this.baseUrl}/admin-stats`);
  ...
  async getSystemHealth(): Promise<SystemHealth> {
    try {
      ...
      const services = [
        ...
        { name: 'Subscription Service', url: 'http://localhost:5002/health' },
        { name: 'Search Service', url: 'http://localhost:5043/health' },
        { name: 'AI Service', url: 'http://localhost:5012/health' },
        { name: 'Document Service', url: 'http://localhost:5003/health' },
      ];
